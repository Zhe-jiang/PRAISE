##########################################################################################################
# realignment for PRAISE no-spliced alignment result, forward sequence
# Author: Zhe Jiang
...
def judge_end_mismatch(sequence_ref, sequence_read, cur_site):
    ...
    for j in range(cur_site, len(sequence_ref)):
        if sequence_read[j] == sequence_ref[j] or (sequence_ref[j] == 'C' and sequence_read[j] == 'T'):
            jud = False
            break
...
def realign(input_temp_bam, ref_dic, max_softclip_temp, min_score_per_base_temp, max_softclip_input_temp, matrix_temp, q_num, q_bool, tot_tasks, lock, rea_mode_rea):
    ...
    for read in allreads:
        ...
        if (not judge_softclip(read.cigarstring, max_softclip_input_temp)) or read.is_unmapped or read.is_reverse:
...
if __name__ == "__main__":
    ...
    for i in range(len(sys.argv)):
        ...
        if input_para == '-i':
            ...
            print('Usage:')
            print('    python realignment_forward.py [Options]* -x <reference> {-i <bam_in>} [-o <bam_out> -f <bam_fil_out>]')
            print('')
