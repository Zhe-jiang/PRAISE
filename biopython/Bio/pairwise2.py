...
from biopython.Bio import BiopythonWarning
...
def _make_score_matrix_generic(
    ...
    for row in range(1, lenA + 1):
        for col in range(1, lenB + 1):
            ...
            if row == lenA:
                row_open = score_matrix[row][col - 1] + gap_A_fn(row, 1, 'N')   # Zhe Jiang, add '+ gap_A_fn(row, 1, 'N')', in case open too much gaps in the end of seq ref
                row_extend = max(score_matrix[row][x] + gap_A_fn(row, col - x, 'N') for x in range(col))    # Zhe jiang, add '+ gap_A_fn(row, col - x, 'N')', in case open too much gaps in the end of seq ref
            else:
                row_open = score_matrix[row][col - 1] + gap_A_fn(row, 1, 'N')
                row_extend = max(
                    score_matrix[row][x] + gap_A_fn(row, col - x, 'N') for x in range(col)
                )
            ...
            if col == lenB:
                ...
                col_open = score_matrix[row - 1][col] + gap_B_fn(col, 1, sequenceA[row - 1])
                col_extend = max(
                    score_matrix[x][col] + gap_B_fn(col, row - x, sequenceA[x]) for x in range(row)
                )
...
# print matrix used for debug, Zhe Jiang
    ...
    while in_process and len(tracebacks) < MAX_ALIGNMENTS:
        ...
        while (row > 0 or col > 0) and not dead_end:
            ...
            if not trace:
                ...
                ali_seqA += sequenceA[row : row + 1]
                ali_seqB += gap_char
                col_gap = False  # Zhe Jiang, revised to False. should be True
        ...
        # set no dead end, used to debug, Zhe Jiang
        # dead_end = False
...
def _find_gap_open(
    ...
    for n in range(target):
        ...
        actual_score = score_matrix[row][col] + gap_fn(index, n + 1, sequenceA[row : row + 1])
...
if __name__ == "__main__":
    from biopython.Bio._utils import run_doctest
